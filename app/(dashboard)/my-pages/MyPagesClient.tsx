...
interface MyPagesClientProps {
  pages: LandingPage[];
}

export function MyPagesClient({ pages: initialPages }: MyPagesClientProps) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      router.push(`/my-pages/${data.id}/edit`);
    } catch (err: unknown) {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          ...(!page.is_active ? { published_at: new Date().toISOString() } : {}),
        })
        .eq("id", page.id)
        .select()
        .single();
      if (error) throw error;
      toast.success(data.is_active ? "Page publiée" : "Page dépubliée");
      setPages((prev) => prev.map((p) => (p.id === page.id ? { ...p, ...data } : p)));
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold">Mes Landing Pages</h1>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => router.push(`/my-pages/${page.id}/edit`)}
                        >
