...
export default function AdminCrmPage() {
  ...
  const crmStats = useMemo(() => {
    ...
    // Trouver les colonnes par position
    const sorted = [...(columns ?? [])].sort(
      (a, b) => (a.position ?? 0) - (b.position ?? 0),
    );
    const discussionCol = sorted[0]; // En discussion
    const relanceCol = sorted[1]; // Relance
    const lienCol = sorted[2]; // Lien envoye
    const callCol = sorted[3]; // Call booke
  ...
  return (
    ...
    >
      ...
      {/* Tab content */}
      <motion.div variants={staggerItem}>
        {view === "pipeline" && <SetterPipelineKanban search={search} />}
        {view === "liste" && <SetterPipelineList search={search} />}
