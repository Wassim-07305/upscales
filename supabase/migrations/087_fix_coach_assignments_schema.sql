-- ============================================================
-- Migration: Fix coach_assignments schema conflicts
-- Description: Removes old migration 076 conflict and ensures proper schema
-- ============================================================

-- Drop old RLS policies from migration 076
DROP POLICY IF EXISTS coach_assign_read ON coach_assignments;
DROP POLICY IF EXISTS coach_assign_admin ON coach_assignments;

-- Drop the old table if it exists with wrong references
-- (it references auth.users instead of profiles)
DROP TABLE IF EXISTS coach_assignments CASCADE;

-- Recreate clean table with proper references
